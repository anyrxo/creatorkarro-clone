...
console.log('🔧 FIXING SITEMAP ROUTE FILES...');

// Find all sitemap and XML route files
const routeFiles = [
  ...glob.sync('src/app/**/*sitemap*.xml/route.ts', { cwd: process.cwd() }),
  ...glob.sync('src/app/**/sitemap.ts', { cwd: process.cwd() }),
  ...glob.sync('src/app/**/robots.ts', { cwd: process.cwd() }),
  ...glob.sync('src/app/rss.xml/route.ts', { cwd: process.cwd() }),
];
...
console.log(`\n✅ SITEMAP ROUTES FIXED: ${fixedFiles} files updated`);
console.log('🎯 All sitemap routes now compatible with static export');